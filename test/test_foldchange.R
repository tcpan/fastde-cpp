...
for ( c in L ) {
    ...
    cat(sprintf("cells.1 n=%d; cells.2 n=%d\n", length(cells.1), length(cells.2)))
...
# seuratperc1[, 1]
# seuratperc2[, 1]
cat(sprintf("seurat output %d X %d\n", nrow(seuratfc), ncol(seuratfc)))
...
cat(sprintf("fastde output %d X %d\n", nrow(fastdefc$fc), ncol(fastdefc$fc)))

comparemat("R vs fastde fc", seuratfc, fastdefc$fc)
comparemat("R vs fastde pct1", seuratperc1, fastdefc$pct.1)
comparemat("R vs fastde pct2", seuratperc2, fastdefc$pct.2)
